...
export interface BagAddOutlineIconProps {
  color?: string
  size?: IconSize
}

export function BagAddOutlineIcon({
  size = 'normal',
  color = COLOR.SECONDARY,
}: BagAddOutlineIconProps) {
  const { getColor } = useTheme()
  const contentColor = getColor?.(color)
  const iconSize = getIconSize(size)
  return (
    <Svg width={iconSize} height={iconSize} viewBox='0 0 24 24' fill='none'>
      <Path d='M12 12V18' stroke={contentColor} stroke-linecap='round' stroke-linejoin='round' />
      <Path d='M15 15H9' stroke={contentColor} stroke-linecap='round' stroke-linejoin='round' />
      <Path
        fill-rule='evenodd'
        clip-rule='evenodd'
